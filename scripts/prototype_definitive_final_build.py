...
final_css_injection = """
<style id="final-art-direction-all-features">
    /* --- ANNOUNCEMENT BAR --- */
    .announcement {
    ...
    .logo-white { opacity: 0.7; z-index: 2; } /* <--- 70% OPACITY RULE */
...
# A. Find the announcement bar and header to wrap them
announcement_html = re.search(r'<div class="announcement">.*?</div>', html, re.DOTALL).group(0)
header_html = re.search(r'<header class="header">.*?</header>', html, re.DOTALL).group(0)

# B. Remove them from the main body content
html = html.replace(announcement_html, '')
html = html.replace(header_html, '')

# C. Create the new combined header structure
new_top_structure = f"""
...
# D. Insert the new structure at the top of the theme wrapper
html = re.sub(r'(<div class="prestige-theme">)', r'\\1' + new_top_structure, html)


# E. Replace the generic hero slogans with the correct ones
...
# F. Inject the final CSS and script
...
print("Definitive All-in-One Final Build complete.")
